...
csv_name = config['output_csv_fp'] + 'output_' + str(raw_report_date) + '_' + nowstr + '.csv'#rename nowstr to tdystr if intended to run daily
#print(csv_name)
filenames = []
try:
...
for file in sorted(os.listdir(dir_path_unread)): # + '//unread//'
    try:
        if file != 'BNPLife_DMA093.pdf':#testing files
            #continue
            pass
        ...
        if file not in filenames:
            ...
            for i,l in enumerate(shares_lines):
                ...
                for kw in (config['shares_filter_kw']+list('%0123456789')):
            ...
            shares_lines = [l for l in shares_lines if not re.search(r'20[10-30]', ' '.join(l))] #filter out dates
            shares_lines = [l for l in shares_lines if not re.search(r'\d{1,2}\\\d{1,2}\\\d{1,2}', ' '.join(l))]
            ...
            shares_lines = [s for s in shares_lines if len(re.sub(regex_percentage, '', s).replace(' ','')) >= 7] #7 subject to change
            ...
            if len(shares_pairs) > 0:
                ...
                if len(shares_pairs) == shares_no and shares_values == sorted(shares_values, reverse=True):
                    print('both correct')
            '''
...
if total_files != 0:
    print('AUMs extracted =', (total_files + not_extracted_files_aum) / total_files * 100, '%')
