...
if csv_gen:
    csv_out = [['filename', 'prod name', 'date', 'AUM']]#, 'currency']]
    # initialize csv file
...
while True:
    for file in os.listdir(dir_path + '//unread//'):
        ...
        # following two if statements were used when i was testing certain/select few files. if True is to retain the indentation structure
        # if file in ['AVIVA_FZ90.pdf']:
        if True:

            # <editor-fold desc="Date collection">
            ...
            # <editor-fold desc="AUM collection">
            ...
            # <editor-fold desc="Name collection">
            ...
            for line in pdf_content_all:
                if '人壽' in line:
                    for kw in config['name_keywords']:
                        if kw in line:
            ...
            for i, line in enumerate(name_lines):
                replace_chars = list('，。、.%/樓：*') + config['name_avoid'] + [file[:len(file) - 4]]
